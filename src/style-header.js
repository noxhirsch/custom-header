...
export const styleHeader = (config, ch, haElem = ha_elements()) => {
    ...
    for (const button in config.button_icons) {
      ...
      if (!config.button_icons[button]) {
        if (button === 'menu') ch.header.menu.icon = 'mdi:menu';
          ...
          ch.header[button].querySelector('ha-icon-button').icon = 'mdi:dots-vertical';
        }
      } else {
        if (button === 'options')
          ch.header[button].querySelector('ha-icon-button').icon = config.button_icons[button];
  ...
  if (config.button_text) {
    for (const button in config.button_text) {
      ...
      const buttonElem =
        button === 'options' ? ch.header[button].querySelector('ha-icon-button') : ch.header[button];
